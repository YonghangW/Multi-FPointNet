...
parser.add_argument('--output_dir', default=None, help='Output directory for detection results')
    ...
    with tf_compat.Graph().as_default():
        with tf_compat.device('/gpu:' + str(GPU_INDEX)):
            ...
            end_points = MODEL.get_model_with_image(
                pointclouds_pl, image_pl, one_hot_vec_pl, is_training_pl, bn_decay=None,
        ...
        for batch_idx in range(num_batches):
            ...
            # 运行预测
            seg_pred_val, center_pred_val, \
            hclass_pred_val, hres_pred_val, \
            sclass_pred_val, sres_pred_val = sess.run([
                seg_pred, center_pred,
                heading_class_pred, heading_residual_pred,
                size_class_pred, size_residual_pred
            ], feed_dict=feed_dict)
        ...
        for class_idx in [0, 1, 2]:
            if len(iou_stats[class_idx]) > 0:
                ...
                print(f"  {CLASS2TYPE[class_idx]}: mean={mean_dist:.3f}, median={median_dist:.3f}")
        print(f"{'='*70}\n")
