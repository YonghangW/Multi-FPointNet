...
parser.add_argument(
    "--model",
    default="frustum_pointnets_v1",
    help="Model name [default: frustum_pointnets_v1]",
)
parser.add_argument(
    "--model_path",
    default="log/model.ckpt",
...
parser.add_argument(
    "--data_path", default=None, help="frustum dataset pickle filepath [default: None]"
)
parser.add_argument(
    "--from_rgb_detection",
    action="store_true",
...
def inference(sess, ops, pc, one_hot_vec, batch_size):
    ...
    heading_res = np.array(
        [heading_residuals[i, heading_cls[i]] for i in range(pc.shape[0])]
    )
    size_res = np.vstack(
        [size_residuals[i, size_cls[i], :] for i in range(pc.shape[0])]
    )
...
def test_from_rgb_detection(output_filename, result_dir=None):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_rot_angle, batch_rgb_prob, batch_one_hot_vec = get_batch(
            TEST_DATASET,
            test_idxs,
            start_idx,
            end_idx,
    ...
    write_detection_results(
        result_dir,
        TEST_DATASET.id_list,
        TEST_DATASET.type_list,
        TEST_DATASET.box2d_list,
        center_list,
        heading_cls_list,
        heading_res_list,
        size_cls_list,
    ...
    if FLAGS.idx_path is not None:
        to_fill_filename_list = [
            line.rstrip() + ".txt" for line in open(FLAGS.idx_path)
        ]
