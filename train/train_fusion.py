...
import provider
from train_util import get_batch_multimodal
...
parser.add_argument(
    "--log_dir", default="log_lite_fusion", help="Log dir [default: log_lite_fusion]"
)
parser.add_argument(
    "--num_point", type=int, default=1024, help="Point Number [default: 1024]"
)
parser.add_argument(
    "--max_epoch", type=int, default=200, help="Epoch to run [default: 200]"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=80000,
    help="Decay step for lr decay [default: 80000]",
)
parser.add_argument(
    "--decay_rate",
    type=float,
...
parser.add_argument(
    "--restore_model_path", default=None, help="Restore model path [default: None]"
)
...
parser.add_argument(
    "--random_flip", action="store_true", help="Enable random_flip augmentation"
)
parser.add_argument(
    "--random_shift", action="store_true", help="Enable random_shift augmentation"
)
parser.add_argument(
    "--train_data_path", default=None, help="Override train pickle path"
)
parser.add_argument("--val_data_path", default=None, help="Override val pickle path")
parser.add_argument(
    "--val_rgb_data_path", default=None, help="Override val rgb-detection pickle path"
)
parser.add_argument(
    "--img_crops_dir", default=None, help="Path to offline image crops directory"
)
...
parser.add_argument(
    "--eval_interval", type=int, default=5, help="Evaluate every N epochs [default: 5]"
)
...
# 数据集路径
TRAIN_DATASET = FLAGS.train_data_path or os.path.join(
    ROOT_DIR, "kitti/frustum_carpedcyc_train.pickle"
)
VAL_DATASET = FLAGS.val_data_path or os.path.join(
    ROOT_DIR, "kitti/frustum_carpedcyc_val.pickle"
)
VAL_DATASET_RGB = FLAGS.val_rgb_data_path or os.path.join(
    ROOT_DIR, "kitti/frustum_carpedcyc_val_rgb_detection.pickle"
)
...
print(f"Random Flip: {FLAGS.random_flip}, Random Shift: {FLAGS.random_shift}")
...
def get_bn_decay(batch):
    ...
    bn_momentum = tf_compat.train.exponential_decay(
        0.5, batch * BATCH_SIZE, DECAY_STEP, DECAY_RATE, staircase=True
    )
    bn_decay = tf_compat.minimum(0.99, 1 - bn_momentum)
    return bn_decay


def evaluate_one_epoch(sess, ops, dataset, num_samples=-1, log_file=None, epoch=0):
    """评估一个epoch，返回各项指标.
    ...
    if log_file is not None:
        log_string(log_file, str(datetime.now()))
        log_string(log_file, "---- EPOCH %03d EVALUATION ----" % (epoch))
    ...
    for batch_idx in range(num_batches):
        ...
        batch_image = batch[1]
        batch_label = batch[2]
        batch_center = batch[3]
        batch_hclass = batch[4]
        batch_hres = batch[5]
        batch_sclass = batch[6]
        batch_sres = batch[7]
        batch_rot_angle = batch[8]
        batch_one_hot_vec = batch[9]
        ...
        loss_val, seg_pred_val, center_pred_val = sess.run(
            [ops["loss"], ops["seg_pred"], ops["center_pred"]], feed_dict=feed_dict
        )
...
def train():
    ...
    # 写入配置
    log_string(
        log_file, f"Training started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    train_dataset = provider.FrustumDatasetMultimodal(
        npoints=NUM_POINT,
        split="train",
        rotate_to_center=True,
        random_flip=FLAGS.random_flip,
        random_shift=FLAGS.random_shift,
        one_hot=True,
        from_rgb_detection=False,
        overwritten_data_path=TRAIN_DATASET,
        img_crops_dir=FLAGS.img_crops_dir,
    )
    val_dataset = provider.FrustumDatasetMultimodal(
        npoints=NUM_POINT,
        split="val",
        rotate_to_center=True,
        one_hot=True,
        from_rgb_detection=False,
        overwritten_data_path=VAL_DATASET,
        img_crops_dir=FLAGS.img_crops_dir,
    )
    ...
    with tf_compat.Graph().as_default():
        with tf_compat.device("/gpu:" + str(GPU_INDEX)):
            ...
            heading_class_label_pl = tf_compat.placeholder(
                tf.int32, shape=(BATCH_SIZE,)
            )
            heading_residual_label_pl = tf_compat.placeholder(
                tf.float32, shape=(BATCH_SIZE,)
            )
            size_class_label_pl = tf_compat.placeholder(tf.int32, shape=(BATCH_SIZE,))
            size_residual_label_pl = tf_compat.placeholder(
                tf.float32, shape=(BATCH_SIZE, 3)
            )
            ...
            pointnet_vars = [
                v for v in train_vars if "image_feature_extractor" not in v.name
            ]
            image_vars = [v for v in train_vars if "image_feature_extractor" in v.name]
            ...
            # 图像网络梯度（缩小学习率）
            image_grads = [
                g * IMAGE_LR_FACTOR if g is not None else None
                for g in grads[len(pointnet_vars) :]
            ]

            # 梯度裁剪
            capped_pointnet_grads = [
                (tf_compat.clip_by_norm(g, 5.0) if g is not None else None, v)
            ...
            with tf_compat.control_dependencies(update_ops):
                train_op = optimizer.apply_gradients(
                    capped_pointnet_grads + capped_image_grads, global_step=batch
                )
        ...
        if FLAGS.restore_model_path is not None:
            ...
            log_string(
                log_file,
                f"Restored from: {FLAGS.restore_model_path}, epoch {start_epoch}",
            )

        # Summary writers
        train_writer = tf_compat.summary.FileWriter(
        ...
        for epoch in range(start_epoch, MAX_EPOCH):
            ...
            if epoch % FLAGS.eval_interval == 0 or epoch == MAX_EPOCH - 1:
                eval_metrics = evaluate_one_epoch(
                    sess, ops, val_dataset, FLAGS.num_val_max
                )
                ...
                summary.value.add(
                    tag="val/seg_acc", simple_value=eval_metrics["seg_acc"]
                )
                ...
                if eval_metrics["seg_acc"] > BEST_ACC:
                    ...
                    log_string(
                        log_file,
                        f"  [*] New best model! Acc: {BEST_ACC:.4f}, saved to: {best_save_path}",
                    )

            # 定期保存模型
            if epoch % 10 == 0 and epoch > 0:
                save_path = saver.save(
                    sess, os.path.join(FLAGS.log_dir, "model.ckpt"), global_step=epoch
                )
        ...
        log_string(
            log_file, f"Best Validation Acc: {BEST_ACC:.4f} at epoch {BEST_EPOCH}"
        )
        log_string(log_file, f"{'=' * 70}")
...
def train_one_epoch(sess, ops, train_writer, dataset, epoch, log_file):
    ...
    for batch_idx in range(num_batches):
        ...
        batch = get_batch_multimodal(
            dataset, train_idxs, start_idx, end_idx, NUM_POINT, NUM_CHANNEL
        )
        batch_data = batch[0]
        batch_image = batch[1]
        batch_label = batch[2]
