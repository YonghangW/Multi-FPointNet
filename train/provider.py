...
class FrustumDatasetMultimodal(FrustumDataset):
    """Multimodal dataset class for Frustum PointNets with image crops.
    Loads both point clouds and corresponding image crops.
    """
    ...
    def __init__(
        ...
        try:
            ...
            print("Warning: OpenCV not available, image loading disabled")
            self.cv2 = None
    ...
    def __getitem__(self, index):
        ...
        if self.from_rgb_detection:
            ...
            if self.one_hot:
                ...
                return point_set, image_crop, rot_angle, prob
        else:
            (
                point_set,
                seg,
    ...
    def get_image_crop(self, index):
        ...
        try:
            ...
            dataset = kitti_object(self.kitti_root)
            calib = dataset.get_calibration(self.id_list[index])
        except:
            pass

        img = self.cv2.imread(
            os.path.join(self.kitti_root, "image_2", self.id_list[index] + ".png")
        )
        if img is None:
            return np.zeros((self.image_size, self.image_size, 3), dtype=np.float32)
        ...
        img_crop = self.cv2.resize(img_crop, (self.image_size, self.image_size))
