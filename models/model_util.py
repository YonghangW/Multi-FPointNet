            ...
            if len(pos_indices) > 0:
                if len(pos_indices) > npoints:
                    ...
                    choice = np.random.choice(
                        len(pos_indices), npoints - len(pos_indices), replace=True
                    )
...
def get_box3d_corners_helper(centers, headings, sizes):
    ...
    N = (
        centers.get_shape()[0]
        if hasattr(centers.get_shape()[0], "value")
        else tf.shape(centers)[0]
    )
    ...
    y_corners = tf.concat(
        [h / 2, h / 2, h / 2, h / 2, -h / 2, -h / 2, -h / 2, -h / 2], axis=1
    )  # (N,8)
    z_corners = tf.concat(
    ...
    R = tf.concat(
        [tf.expand_dims(row1, 1), tf.expand_dims(row2, 1), tf.expand_dims(row3, 1)],
        axis=1,
    )  # (N,3,3)
...
def get_box3d_corners(center, heading_residuals, size_residuals):
    ...
    batch_size = (
        center.get_shape()[0]
        if hasattr(center.get_shape()[0], "value")
        else tf.shape(center)[0]
    )
    heading_bin_centers = tf.constant(
        np.arange(0, 2 * np.pi, 2 * np.pi / NUM_HEADING_BIN), dtype=tf.float32
    )  # (NH,)
...
def parse_output_to_tensors(output, end_points):
    ...
    batch_size = (
        output.get_shape()[0]
        if hasattr(output.get_shape()[0], "value")
        else tf.shape(output)[0]
    )
    ...
    heading_residuals_normalized = tf.slice(
        output, [0, 3 + NUM_HEADING_BIN], [-1, NUM_HEADING_BIN]
    )
    ...
    size_scores = tf.slice(
        output, [0, 3 + NUM_HEADING_BIN * 2], [-1, NUM_SIZE_CLUSTER]
    )  # BxNUM_SIZE_CLUSTER
    size_residuals_normalized = tf.slice(
        output,
        [0, 3 + NUM_HEADING_BIN * 2 + NUM_SIZE_CLUSTER],
        [-1, NUM_SIZE_CLUSTER * 3],
    )
    size_residuals_normalized = tf.reshape(
        size_residuals_normalized, [batch_size, NUM_SIZE_CLUSTER, 3]
    )  # BxNUM_SIZE_CLUSTERx3
    end_points["size_scores"] = size_scores
    end_points["size_residuals_normalized"] = size_residuals_normalized
    end_points["size_residuals"] = size_residuals_normalized * tf.expand_dims(
...
def placeholder_inputs_with_image(batch_size, num_point, img_height=224, img_width=224):
    """Get useful placeholder tensors for multimodal model.
    Input:
        batch_size: scalar int
        num_point: scalar int
    ...
    """
    pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point, 4))
    image_pl = tf.placeholder(tf.float32, shape=(batch_size, img_height, img_width, 3))
    one_hot_vec_pl = tf.placeholder(tf.float32, shape=(batch_size, 3))
...
def point_cloud_masking(point_cloud, logits, end_points, xyz_only=True):
    ...
    mask = tf.slice(logits, [0, 0, 0], [-1, -1, 1]) < tf.slice(
        logits, [0, 0, 1], [-1, -1, 1]
    )
    mask = tf.cast(mask, tf.float32)  # BxNx1
    mask_count = tf.tile(tf.reduce_sum(mask, axis=1, keepdims=True), [1, 1, 3])  # Bx1x3
    point_cloud_xyz = tf.slice(point_cloud, [0, 0, 0], [-1, -1, 3])  # BxNx3
    mask_xyz_mean = tf.reduce_sum(
        tf.tile(mask, [1, 1, 3]) * point_cloud_xyz, axis=1, keepdims=True
    )  # Bx1x3
    ...
    num_channels = (
        point_cloud_stage1.get_shape()[2]
        if hasattr(point_cloud_stage1.get_shape()[2], "value")
        else tf.shape(point_cloud_stage1)[2]
    )

    object_point_cloud, _ = tf_gather_object_pc(
        point_cloud_stage1, mask, NUM_OBJECT_POINT
    )

    # Handle dynamic shape for set_shape
    # Try to set static shape if available
    try:
        bs = batch_size.value if hasattr(batch_size, "value") else None
        nc = num_channels.value if hasattr(num_channels, "value") else None
        if bs is not None and nc is not None:
            object_point_cloud.set_shape([bs, NUM_OBJECT_POINT, nc])
    except:
        pass
...
def get_center_regression_net(
    ...
    net = tf_util.max_pool2d(
        net, [num_point, 1], padding="VALID", scope="maxpool-stage1"
    )
    ...
    predicted_center = tf_util.fully_connected(
        net, 3, activation_fn=None, scope="fc3-stage1"
    )
...
def get_loss(
    ...
    hcls_onehot = tf.one_hot(
        heading_class_label, depth=NUM_HEADING_BIN, on_value=1, off_value=0, axis=-1
    )  # BxNUM_HEADING_BIN
    heading_residual_normalized_label = heading_residual_label / (
        np.pi / NUM_HEADING_BIN
    )

    # Fix: Use tf.cast instead of tf.to_float for TF2 compatibility
    hcls_onehot_float = tf.cast(hcls_onehot, tf.float32)
    heading_residual_normalized_loss = huber_loss(
        tf.reduce_sum(
            end_points["heading_residuals_normalized"] * hcls_onehot_float,
            axis=1,
        )
    ...
    tf.summary.scalar(
        "heading residual normalized loss", heading_residual_normalized_loss
    )
    ...
    scls_onehot = tf.one_hot(
        size_class_label, depth=NUM_SIZE_CLUSTER, on_value=1, off_value=0, axis=-1
    )  # BxNUM_SIZE_CLUSTER

    # Fix: Use tf.cast instead of tf.to_float for TF2 compatibility
    scls_onehot_float = tf.cast(scls_onehot, tf.float32)
    scls_onehot_tiled = tf.tile(
        tf.expand_dims(scls_onehot_float, -1), [1, 1, 3]
    )  # BxNUM_SIZE_CLUSTERx3
    predicted_size_residual_normalized = tf.reduce_sum(
        end_points["size_residuals_normalized"] * scls_onehot_tiled, axis=[1]
    )  # Bx3

    mean_size_arr_expand = tf.expand_dims(
        tf.constant(g_mean_size_arr, dtype=tf.float32), 0
    )  # 1xNUM_SIZE_CLUSTERx3
    mean_size_label = tf.reduce_sum(
        scls_onehot_tiled * mean_size_arr_expand, axis=[1]
    )  # Bx3
    ...
    # Fix: Use tf.cast instead of tf.to_float for TF2 compatibility
    gt_mask = tf.tile(
        tf.expand_dims(hcls_onehot, 2), [1, 1, NUM_SIZE_CLUSTER]
    ...
    corners_3d_pred = tf.reduce_sum(
        tf.expand_dims(tf.expand_dims(gt_mask_float, -1), -1) * corners_3d,
        axis=[1, 2],
    )  # (B,8,3)
    ...
    heading_label = tf.expand_dims(heading_residual_label, 1) + tf.expand_dims(
        heading_bin_centers, 0
    )  # (B,NH)

    # Fix: Use tf.cast instead of tf.to_float for TF2 compatibility
    hcls_onehot_float = tf.cast(hcls_onehot, tf.float32)
    heading_label = tf.reduce_sum(hcls_onehot_float * heading_label, 1)

    mean_sizes = tf.expand_dims(
        tf.constant(g_mean_size_arr, dtype=tf.float32), 0
    )  # (1,NS,3)
    size_label = mean_sizes + tf.expand_dims(
        size_residual_label, 1
    )  # (1,NS,3) + (B,1,3) = (B,NS,3)

    # Fix: Use tf.cast instead of tf.to_float for TF2 compatibility
    scls_onehot_float = tf.cast(scls_onehot, tf.float32)
    size_label = tf.reduce_sum(
        tf.expand_dims(scls_onehot_float, -1) * size_label, axis=[1]
    )  # (B,3)
    ...
    # Note: tf.add_to_collection is deprecated in TF2
    # For TF2, losses should be handled differently
