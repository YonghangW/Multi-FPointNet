"""Frsutum PointNets v1 Model with Image Feature Fusion (TF2 Compatible)."""
...
from image_feature_extractor import get_image_feature_extractor, get_image_feature_v2


def get_instance_seg_v1_net_with_image(
    point_cloud, image_features, one_hot_vec, is_training, bn_decay, end_points
):
    """3D instance segmentation PointNet v1 network with image features.
    Input:
        point_cloud: TF tensor in shape (B,N,4)
        image_features: TF tensor in shape (B, 512) - image features (updated to 512-dim)
    ...
    global_feat = tf_util.max_pool2d(
    ...
    one_hot_expanded = tf.expand_dims(tf.expand_dims(one_hot_vec, 1), 1)
    one_hot_expanded = tf.tile(one_hot_expanded, [1, num_point, 1, 1])

    concat_feat = tf.concat(
        axis=3, values=[point_feat, global_feat_expand, one_hot_expanded]
    )
...
def get_3d_box_estimation_v1_net_with_image(
    object_point_cloud, image_features, is_training, bn_decay, end_points
):
    """3D Box Estimation PointNet v1 network with image features.
    Input:
        object_point_cloud: TF tensor in shape (B,M,C)
        image_features: TF tensor in shape (B, 512) - image features
    Output:
        output: TF tensor
    """
    num_point = (
    ...
    # Concatenate 512-dim image features with point cloud features (no one-hot)
    net = tf.concat([net, image_features], axis=1)

    # First FC layer outputs 512-dim to match image features
    net = tf_util.fully_connected(
        net, 512, scope="fc1", bn=True, is_training=is_training, bn_decay=bn_decay
    )
    net = tf_util.fully_connected(
...
def get_3d_box_estimation_v1_net(
...
def get_model_with_image(
    ...
    """Frustum PointNets model with image feature fusion.
    This model fuses ResNet-50 + Transformer features with point cloud features.

    Key updates for TF2:
    - Uses tf.keras.applications.ResNet50 for image feature extraction
    - Transformer encoder implemented with TF2 Keras layers
    - Image features: 512 dimensions
    - Point cloud features: 512 dimensions (from conv-reg4)
    - Fusion at 3D box estimation stage

    Input:
        point_cloud: TF tensor in shape (B,N,4)
            frustum point clouds with XYZ and intensity in point channels
        image_input: TF tensor in shape (B, H, W, 3)
            RGB image crops from frustum region
        one_hot_vec: TF tensor in shape (B,3)
            length-3 vectors indicating predicted object type
        is_training: TF boolean scalar
        bn_decay: TF float scalar
    ...
    # Extract 512-dim image features using ResNet-50 + Transformer
    # Note: Using get_image_feature_extractor instead of get_image_feature_v2
    # to get the full 512-dim output
    ...
    logits, end_points = get_instance_seg_v1_net_with_image(
        point_cloud, image_features, one_hot_vec, is_training, bn_decay, end_points
    )
    end_points["mask_logits"] = logits
...
if __name__ == "__main__":
    ...
    print("Testing Frustum PointNets v1 Model (TF2 Compatible)...")
