...
def get_3d_box_estimation_v1_net_with_image(
    ...
    Input:
        object_point_cloud: (B,M,C) - 分割后的点云 (xyz 或 xyz+feature)
...
def _prepare_point_cloud(point_cloud, use_feature_channel):
    """Optionally drop extra feature channel (e.g., Gauss mask)."""
    if use_feature_channel:
        return point_cloud
    return tf.slice(point_cloud, [0, 0, 0], [-1, -1, 3])


def _combine_xyz_with_feature(
    object_point_cloud, object_point_cloud_xyz_new, use_feature_channel
):
    if not use_feature_channel:
        return object_point_cloud_xyz_new
    feature = tf.slice(object_point_cloud, [0, 0, 3], [-1, -1, -1])
    return tf.concat([object_point_cloud_xyz_new, feature], axis=2)
...
def get_model_with_image(
    ...
    image_freeze_at=0,
    use_feature_channel=True,
    ablation_mode="gauss_image",
):
    ...
    # 3D实例分割（只用点云）
    point_cloud_in = _prepare_point_cloud(point_cloud, use_feature_channel)
    logits, end_points = get_instance_seg_v1_net(
        point_cloud_in, one_hot_vec, is_training, bn_decay, end_points
    )
    end_points["mask_logits"] = logits

    # 根据分割mask提取目标点云
    object_point_cloud_xyz, mask_xyz_mean, end_points = point_cloud_masking(
        point_cloud_in, logits, end_points, xyz_only=not use_feature_channel
    )

    if use_feature_channel:
        object_point_cloud = object_point_cloud_xyz
    ...
    object_point_cloud_stage1 = _combine_xyz_with_feature(
        object_point_cloud, object_point_cloud_xyz_new, use_feature_channel
    )

    # 3D框估计（融合图像特征）
    if ablation_mode == "image_only":
        image_features_in = image_features
    elif ablation_mode == "gauss_image":
        image_features_in = image_features
    else:
        image_features_in = tf.zeros_like(image_features)

    output, end_points = get_3d_box_estimation_v1_net_with_image(
        object_point_cloud_stage1,
        image_features_in,
        one_hot_vec,
        is_training,
        bn_decay,
...
# 保持原始模型兼容性
def get_model(
    point_cloud, one_hot_vec, is_training, bn_decay=None, use_feature_channel=True
):
    ...
    # 原始回归网络（无图像）
    num_point = (
        object_point_cloud_stage1.get_shape()[1]
        if hasattr(object_point_cloud_stage1.get_shape()[1], "value")
        else tf.shape(object_point_cloud_stage1)[1]
    )
    net = tf.expand_dims(object_point_cloud_stage1, 2)
    net = tf_util.conv2d(
        net,
        128,
        [1, 1],
